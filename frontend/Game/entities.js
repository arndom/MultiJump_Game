...
export class Ball extends Entity {
    ...
    update() {
        ...
        if (this.canThrow && startCountdown <= 0 && gameTimer > 0) {
            this.handleTouch();
        }
        ...
        //Check collision with both hoop sides while falling
        if (this.falling) {
            ...
            if (this.pos.y > hoopSides[0].pos.y + objSize / 2) {
                if (this.pos.x > hoopSides[0].pos.x && this.pos.x < hoopSides[1].pos.x) {
                    if (!this.scored) {
                        ...
                        if (!Koji.config.settings.ballCanPassThrough) {
                            spawnBall();
                        }
    ...
    handleTouch() {
        //Drag the ball while isTouching
        if (isTouching) {
            if (this.idle) {
...
export class Hoop extends Entity {
    ...
    update() {
        ...
        if (this.animTimer < 1) {
            this.animTimer += 1 / frameRate() * 5;


            let offset = 1.5;
            this.scale.x = Ease(EasingFunctions.easeOutElastic, this.animTimer, 1 + offset, -offset);
        }

        //Always tend to have scale of 1
        //this.scale.x = Smooth(this.scale.x, 1, 8);
        //this.scale.y = Smooth(this.scale.y, 1, 8);


        this.pos.x = boardPos; //move with board
    }
}

//Hoop sides, important for collisions with ball
export class HoopSide extends Entity {
    constructor(x, y) {
        super(x, y);
...
export class Guide extends Entity{
  ...
  update(){
    ...
    if(this.animTimer < 1){
      this.animTimer += 1/frameRate() * 0.75;
    }else{
      this.animTimer = 0;
    }


    this.pos.y = Ease(EasingFunctions.easeOutQuint, this.animTimer, this.startY, -this.moveAmount);
